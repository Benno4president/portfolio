  ...
  let img = $("<img>")
    ...
    .on("load", function () {
      ...
      if (
        ...
        console.warn(
          "Error loading hero image! Might not exist for this recipe, but if it does make sure the filename is the same as the recipe file and has a .jpg extension"
        );
      } else {
    ...
    success: function (recipe) {
      ...
      for (let i in sections) {
        ...
        if (!foundTitle) {
          ...
          let elems = $(section);
          let pageTitle = elems[0].innerHTML + " | Portfolio";
      ...
      $("#info ul").html(
        '<li><span id="time">TIME </span>' +
          time.text() +
          '</li><li><span id="makes">MAKES </span>' +
          makes.text() +
          "</li>"
      );
      ...
      let linkIcon =
        '<svg class="linkIcon" aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">';
      linkIcon +=
      ...
      for (let j in helpUrls) {
        ...
        help +=
          '<li><a href="' + url + '" target="blank">' + label + "</a></li>";
      }
      help += "</ul>";
      $("#help").html(help);
